...
namespace CodeGeneration.Kubernetes
{
        ...
        public static IReadOnlyCollection<Definition> LoadDefinitions(string definitionPath)
        {
            ...
            var definitions = Directory.GetFiles(definitionPath)
                ...
                .Select(x =>
                {
                    try
                    {
                        var obj = deserializer.Deserialize<object>(x);
                        return JObject.FromObject(obj).ToObject<Definition>();
                    }
                    catch (Exception ex)
                    {
    ...
    internal static class DefinitionMarkdownParser
    {
        public static Definition Parse(IReadOnlyList<string> definitionLines)
        {
            ...
            for (var i = 0; i < definitionLines.Count; i++)
            {
                ...
                if (line.StartsWith("##"))
                {
                    if (regionName != null)
                    {
                        regions.Add(regionName, definitionLines.Skip(regionFirstLine).Take(i - 1 - regionFirstLine).ToArray());
                    }

                    regionName = line.Trim(new[] { ' ', '#' });
                    regionFirstLine = i + 1;
                }
            }
        ...
        private static Definition ParseRegions(IReadOnlyDictionary<string, IReadOnlyList<string>> regions)
        {
            ...
            definition.Description = synopsisRegion.TakeWhile(x => !x.StartsWith("```")).Where(x => !string.IsNullOrWhiteSpace(x)).JoinNewLine(PlatformFamily.Linux);
            definition.Synopsis = synopsisRegion
                .SkipWhile(x => !x.StartsWith("```"))
                .Skip(count: 1)
                .TakeWhile(x => x.StartsWith("```"))
                .JoinNewLine(PlatformFamily.Linux);
            definition.Options = ParseOptions(regions["Options"]);
            definition.InheritedOptions = regions.TryGetValue("Options inherited from parent commands", out var inheritedOptions) ? ParseOptions(inheritedOptions) : new Option[0];
        ...
        private static Option[] ParseOptions(IEnumerable<string> lines)
        {
            ...
            lines.Where(x => !string.IsNullOrWhiteSpace(x) && !x.StartsWith("```"))
                ...
                .ForEach((line, index) =>
                {
                    ...
                    option.Description = match.Groups["description"].Value;
                    options.Add(option);
                });
